...
version="latest"
distribution_repo="Cardinal85/frp-panel-distribution"
...
while [[ "$#" -gt 0 ]]; do
    case "$1" in
        ...
        --download-base)
            if [ -z "$2" ]; then
                echo "Error: --download-base requires a URL."
                exit 1
            fi
            download_base="$2"
            echo "使用自定义客户端下载地址: $download_base"
            shift 2
            ;;
        *)
...
release_url() {
    local file="$1"
    local base="${download_base:-https://github.com/${distribution_repo}/releases}"
    local release_platform="linux"
    local release_arch="$ARCH"
    [ "$OS" = "Darwin" ] && release_platform="darwin"
    [ "$ARCH" = "x86_64" ] && release_arch="amd64"
    [ "$ARCH" = "aarch64" ] && release_arch="arm64"
    base="${base%/}"

    if [[ "$base" == *"{version}"* || "$base" == *"{file}"* || "$base" == *"{platform}"* || "$base" == *"{arch}"* ]]; then
        base="${base//\{version\}/$version}"
        base="${base//\{file\}/$file}"
        base="${base//\{platform\}/$release_platform}"
        base="${base//\{arch\}/$release_arch}"
        echo "$base"
    elif [ -n "$download_base" ]; then
        echo "$base/$file"
    elif [ "$version" = "latest" ]; then
        echo "https://github.com/${distribution_repo}/releases/latest/download/$file"
    else
        echo "https://github.com/${distribution_repo}/releases/download/$version/$file"
    fi
}
...
case "$OS" in
    Linux)
        case "$ARCH" in
            x86_64)
                wget -O frp-panel "${prefix}$(release_url frp-panel-linux-amd64)"
                ;;
            aarch64)
                wget -O frp-panel "${prefix}$(release_url frp-panel-linux-arm64)"
                ;;
            armv7l)
                wget -O frp-panel "${prefix}$(release_url frp-panel-linux-armv7l)"
                ;;
            armv6l)
                wget -O frp-panel "${prefix}$(release_url frp-panel-linux-armv6l)"
                ;;
            *)
    ...
    Darwin)
        case "$ARCH" in
            x86_64)
                wget -O frp-panel "${prefix}$(release_url frp-panel-darwin-amd64)"
                ;;
            arm64)
                wget -O frp-panel "${prefix}$(release_url frp-panel-darwin-arm64)"
                ;;
            *)
